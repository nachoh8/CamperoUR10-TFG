...
def mouse_callback(event, x, y, flags, param):
    global pub, can_update, blur_ksize, contour_method, min_contour_size, apply_sharp, max_dist_error, binary_thresh, conectivity_way, number_iterations, apply_concaveman, concaveman_alpha, apply_smooth_path, smooth_path_kernel
    if pub is None:
...
def trackbar_callback(idx, v):
    global blur_ksize, erode_type, erode_size, dilate_type, dilate_size, contour_method, min_contour_size, apply_sharp, max_dist_error, binary_thresh, conectivity_way, number_iterations, apply_concaveman, concaveman_alpha, apply_smooth_path, smooth_path_kernel
    if idx == 2:
        blur_ksize = v
    elif idx == 3:
        erode_type = v
...
img_settings = cv.putText(img_settings, 'Update', text_pos, cv.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv.LINE_AA)
...
img_settings = cv.putText(img_settings, 'Process', text_pos, cv.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv.LINE_AA)
