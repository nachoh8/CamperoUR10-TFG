#include "../include/draw_board/utils.h"
#include "../include/draw_board/img_params.hpp"
#include "../include/draw_board/img_board.hpp"
...
static const std::string NODE_NAME = "draw_board";
...
private:
    ...
    std::unique_ptr<ImageBoard> best_img;
...
public:
    DrawBoard() : it(nh) {
        markers_img_sub = nh.subscribe("/aruco_detector/markers_img", 1, &DrawBoard::markers_img_callback, this);
        cam_info_sub = nh.subscribe("/camera/color/camera_info", 1, &DrawBoard::cam_info_callback, this);
        process_cmd_sub = nh.subscribe("/" + NODE_NAME + "/cmd", 1, &DrawBoard::process_cmd_callback, this);
    ...
    void markers_img_callback(const campero_ur10_msgs::ArucoMarkersImg& msg) {
        ...
        try {
            ...
            ImageBoard* img_p = new ImageBoard(cv_ptr->image, msg.markers.markers, img_params);
            ...
            ROS_INFO("--New best image board--");
			ROS_INFO(" %s ", img_p->getPrintInfo().c_str());

            img_p->board_main();

            if (img_p->numTotalPoints() == 0) {
                delete img_p;
                return;
            }
			...
			ROS_INFO("Num trazos: %d ", best_img->numberCountours());
            ROS_INFO("Num total puntos: %d ", best_img->numTotalPoints());
